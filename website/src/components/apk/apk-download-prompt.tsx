    ...
    <Dialog open={open} onOpenChange={(next) => !next && onDismiss()}>
      ...
      >
        <div className="relative">
          ...
          <StarLattice
            tile={52}
            className="absolute inset-x-0 top-0 h-24 w-full text-gold opacity-15"
          />
          ...
          <button
            ...
            className="focus-ring absolute right-3 top-3 z-10 rounded-full bg-background/80 p-1.5 text-muted-foreground transition-colors hover:bg-background hover:text-foreground"
          >
          ...
          <div className="relative px-6 pb-6 pt-8 text-center sm:px-8">
            ...
            />
            <DialogTitle className="mt-3 font-serif text-2xl font-semibold tracking-tight">
              {brand.name}
            </DialogTitle>
            <DialogDescription
              id="apk-prompt-desc"
                ...
                <li key={benefit} className="flex items-start gap-2.5">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-secondary">
                    {i === 0 ? (
                      ...
                      <Search className="h-3 w-3 text-secondary-foreground" aria-hidden="true" />
                    ) : i === 2 ? (
